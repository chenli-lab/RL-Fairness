...
#define size 2//10201                    // # of nodes (= L*L)
#define tmax 100001000                          // max nr. of time steps
int main()
{
	int U,O,n_ave,realization;
	double LLP[8][8],MMP[8][8],HHP[8][8],LLQ[8][8],MMQ[8][8],HHQ[8][8];
...
//	high = 0.7; 
	...
	make1Darray(State_P,N);                                   //the number of players with +1 state
	make1Darray(State_R,N);       
    make1Darray(State_new_P,N);                                   //the number of players with +1 state
    make1Darray(State_new_R,N);                                   //the number of players with +1 state
    make1Darray(Action_P,N);                                  //action for each player
    make1Darray(Action_R,N);  
    make1Darray(Reward_P,N);                                  //the Reward obtained
    make1Darray(Reward_R,N);                                  //the Reward obtained
	make3Darray(Qtable_P,N,num_state,num_action);             //Q table for each individual
	make3Darray(Qtable_R,N,num_state,num_action);             //Q table for each individual
...
for(U=0;U<8;U++)
{
	low = U*0.05+0.1;    //Adjust the value of l
	for(O=0;O<8;O++)
	{
		high = O*0.05+0.6;    //Adjust the value of h
			...
			for(t=1;t<tmax;t++)
			{
				...
				for(i=0;i<N;i++)
				{
					...
					else                                 
					{
						...
						for(k=0;k<num_action;k++)                               
						{
						    if(Qtable_P[i][State_P[i]][k] > Q_max_P)                    
						    {
					...
					else
					{
						...
						for(k=0;k<num_action;k++)                                
						{
						    if(Qtable_R[i][State_R[i]][k] > Q_max_R)                   
						    {
